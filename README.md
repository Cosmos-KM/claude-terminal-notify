...
| 承認待ち・**他のタブを見ているとき** | 暖色で全面がゆっくり明滅する（作業中と同じ色を濃くしたもの） |
...
- **色は RGB を 0〜65535 の3つの数値**で書きます（`(26000 13000 0)` = 黄みの暖色）。
...
| 設定 | 既定 | 意味 |
| --- | --- | --- |
| `WORKING_TINT` / `WORKING_MIX` | `(26000 13000 0)` / `16` | 作業中の色と濃さ |
| `DONE_TINT` / `DONE_MIX` | `(0 34000 10000)` / `20` | 完了の色と濃さ |
| `ATTENTION_TINT` / `ATTENTION_MIX` | `(26000 13000 0)` / `55` | 承認待ち・**他のタブを見ているとき**の色と濃さ（いちばん明るいとき）。作業中と同じ色なので、濃さで見分ける |

**承認待ち・他のタブを見ているとき**

平常色と暖色の間を、なめらかに行き来します（`fade`）。
...
| `ATTENTION_CURSOR` | `(65535 28000 0)` | 明滅させるカーソルの色（明るい側。暗い側は平常のカーソル色） |
